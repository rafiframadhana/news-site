...
import { Button, Card, Badge, Modal, LoadingSpinner, Input } from '../../components/ui/index.jsx';
...
const ManageUsers = () => {
  ...
  const getRoleBadge = (role) => {
    ...
    return (
      <Badge variant={variants[role] || 'default'} size="sm" className="flex items-center">
        {icons[role]}
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="mb-8 border-b pb-6">
          ...
          <p className="text-gray-600">Manage user accounts and role permissions across the platform</p>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-8 text-center">
          ...
          <p className="text-gray-600 max-w-md mx-auto mb-6">
            There was a problem retrieving user data. Please try again or contact support if the issue persists.
          </p>
          <Button 
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      {users.length === 0 ? (
        ...
        <div className="space-y-4">
          {users.map((user) => (
            <Card key={user._id} padding="md" hover className="shadow-sm transition-all duration-200 hover:shadow-md py-2 px-4">
                ...
                <div className="flex items-center space-x-6 text-sm">
                  {user.role === 'author' && (
                    <div className="text-center px-4 py-2 bg-gray-50 rounded-lg">
                      <div className="font-medium text-gray-900">{user.articlesCount || 0}</div>
      ...
      >
        <div className="space-y-6">
          <div className="flex items-center space-x-4 p-4 bg-red-50 rounded-lg border border-red-100">
            ...
            <div>
              <h3 className="text-lg font-medium text-red-800">Delete confirmation</h3>
              <p className="mt-1 text-sm text-red-700">
                Are you sure you want to delete "{userToDelete?.fullName || userToDelete?.name || userToDelete?.username}"? 
                This action cannot be undone and will also delete all their articles and content.
              </p>
            </div>
          </div>
          ...
          {userToDelete?.role === 'admin' && (
            <div className="p-4 bg-yellow-50 rounded-lg border border-yellow-100">
              <p className="text-sm text-yellow-700">
                <span className="font-bold">Warning:</span> You are about to delete an admin user. Make sure this action is authorized.
              </p>
            </div>
          )}
      ...
      >
        <div className="space-y-6">
          <div className="flex items-start space-x-4">
                ...
                <div className="w-12 h-12 bg-gradient-to-br from-blue-500 to-blue-700 rounded-full flex items-center justify-center text-white">
                  <span className="text-lg font-medium">
                    {userToUpdateRole.firstName 
                      ? `${userToUpdateRole.firstName[0]}${userToUpdateRole.lastName ? userToUpdateRole.lastName[0] : ''}` 
            ...
            <div>
              <h3 className="text-lg font-medium text-gray-900">
                {userToUpdateRole?.fullName || userToUpdateRole?.name || userToUpdateRole?.username}
              </h3>
              <p className="text-sm text-gray-600">
                {userToUpdateRole?.email}
              </p>
              <p className="text-sm text-gray-600 mt-1">
                Current role: <span className="font-medium">{userToUpdateRole?.role}</span>
              </p>
            </div>
          </div>
          ...
          {newRole === 'admin' && userToUpdateRole?.role !== 'admin' && (
            <div className="p-4 bg-yellow-50 rounded-lg border border-yellow-100">
              <p className="text-sm text-yellow-700">
                <span className="font-bold">Caution:</span> You are about to grant full administrative privileges to this user.
              </p>
            </div>
          )}
